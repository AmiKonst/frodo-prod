import { _ as _export_sfc, r as reactive, x as ref, w as watch, o as onMounted, c as createElementBlock, f as openBlock, F as Fragment, m as renderList, s as stores, u as useI18n, b as createBlock, k as withCtx, l as createBaseVNode, e as createVNode, B as Button, P as createTextVNode, v as toDisplayString, K as BaseModal } from './index-ClP55Rim.js';
...
const _sfc_main$1 = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    const onInput = (e, id) => {
        ...
        if (val.length > 1) {
            const digits = val.split('').filter(char => /^\d$/.test(char)).slice(0, props.count);
            
            digits.forEach((char, index) => {
                if (id + index < props.count) {
                    data.values[id + index] = char;
                }
            });

            const nextIdx = Math.min(id + digits.length, props.count - 1);
            focusInput(nextIdx);
            return;
        }
        ...
        if (/^[0-9]$/.test(val)) {
            data.values[id] = +e.target.value;
            // data.values[id] = val; // Убрал +, храним как строку
            nextInput(id);
        } else {
            data.values[id] = '';
        }
...
const Code = /*#__PURE__*/_export_sfc(_sfc_main$1, [['__scopeId',"data-v-23c99cb5"]]);
...
//# sourceMappingURL=ModalCode-DZUI1J8t.js.map
